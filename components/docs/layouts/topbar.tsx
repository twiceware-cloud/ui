...
export const Topbar = ({ menuItems }: TopbarProps) => {
    ...
    return (
        <div className="bg-background/80 dark:bg-background/90 sticky top-0 z-10 flex h-16 items-center justify-between backdrop-blur-sm">
            <div className="flex items-center gap-2 md:gap-8">
                <Drawer.Root direction="left" open={openDrawer} onOpenChange={setOpenDrawer}>
                    <Drawer.Trigger asChild>
                        <Button variant="ghost" size="icon" className="md:hidden" aria-label="Leftmenu toggle">
                            <MenuIcon />
                        </Button>
                    </Drawer.Trigger>
                    <Drawer.Portal>
                ...
                <div className="hidden gap-2 md:inline-flex md:gap-6">
                    <Link
                        className={cn("text-foreground/80 hover:text-foreground text-sm transition-all", {
                            "text-foreground font-medium": pathname.includes("/docs/components"),
                        })}
