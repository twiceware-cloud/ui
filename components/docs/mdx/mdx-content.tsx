...
const sharedComponents = {
    h2: ({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
        <h2
            className={cn("border-border mt-10 scroll-m-16 border-b pb-2 text-xl font-medium sm:text-2xl", className)}
            {...props}
        />
    ),
    ...
    td: ({ className, ...props }: React.HTMLAttributes<HTMLTableCellElement>) => (
        <td
            className={cn(
                "border-e px-4 py-2 text-left text-nowrap last:border-e-0 [&[align=center]]:text-center [&[align=right]]:text-end",
                className,
            )}
            {...props}
        />
