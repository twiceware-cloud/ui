      ...
      {composeRenderProps(children, (children) => (
        <div className={cn('flex gap-2', size === 'icon' ? 'mx-auto' : '')}>
          {!loading && icon && (
            <HugeiconsIcon
              icon={icon}
              className={cn(isToolbar ? 'text-primary' : '', disabled ? 'text-muted-foreground' : '', iconSizeClass, iconClassName)}
            />
          )}
          {loading && <LoaderCircleIcon className={cn('animate-spin', iconSizeClass)}  aria-hidden="true"/>}
