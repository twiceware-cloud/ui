...
const ShowDemo = ({ demo: Demo, href }: { demo: () => ReactNode; href: string }) => {
    ...
    return (
        <div className="group/demo relative flex justify-center px-16 py-32">
            <Demo key={key} />
    ...
    return (
        <div className="my-12">
            <div className="flex flex-col items-center justify-center">
                <p className="from-foreground to-foreground/60 w-fit bg-linear-to-r bg-clip-text text-2xl font-[650] tracking-tight text-transparent lg:text-3xl xl:text-4xl 2xl:text-5xl">
                    Demos
                </p>
                <p className="text-muted-foreground mt-1">Preview interactive examples and see them in action.</p>
            </div>
            <div className="mt-16 grid grid-cols-2 *:border *:border-e-0 *:border-b-0 *:border-dashed *:first:border-t-0 *:odd:border-s-0 [&>*:nth-child(2)]:border-t-0">
                {demos.map((demo, i) => (
                    <ShowDemo {...demo} key={i} />
                ))}
            </div>
